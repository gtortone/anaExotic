#ifndef EXOTIC_UTILS_ODB_H
#define EXOTIC_UTILS_ODB_H
	...
	static bool ReadValue(const char* path, T& value)
		{
			...
			if ( ReadArray(path, &value, sizeof(value)) != 0 )
				...
				return false;
#else
			exotic::utils::Error("midas::Odb::ReadValue", __FILE__, __LINE__)
	...
	static int ReadArray(const char* path, T* array, int length)
		{
			...
			if (status == DB_NO_KEY) {
				exotic::utils::Error("midas::Odb::ReadArray") << "Couldn't get array key for path \""
                                                              << path << "\". status = " << status
                                                              << EXOTIC_ERR_FILE_LINE;
				return 0;
			}
			...
			exotic::utils::Error("midas::Odb::ReadArray", __FILE__, __LINE__)
...
template<> inline bool Odb::ReadValue<std::string>(const char* path, std::string& value)
{
	...
	status = db_find_key (hdb, hdir, path, &hkey);
	if (status == DB_NO_KEY) {
		exotic::utils::Error("midas::Odb::ReadValue<std::string>", __FILE__, __LINE__)
...
template<> inline int Odb::ReadArray<std::string>(const char* path, std::string* array, int length)
{
  ...
  status = db_find_key (hdb, hdir, path, &hkey);
  if (status == DB_NO_KEY) {
    exotic::utils::Error("midas::Odb::ReadArray<std::string>", __FILE__, __LINE__)
      << "Couldn't get array key for path \"" << path << "\". status = " << status;
    return 0;
  }
