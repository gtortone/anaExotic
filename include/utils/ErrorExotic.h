#ifndef EXOTIC_ERROR_HXX
#define EXOTIC_ERROR_HXX
...
class ChangeErrorIgnore {
 public:
   ...
   ChangeErrorIgnore(int level) : fOldLevel(gErrorIgnoreLevel) { gErrorIgnoreLevel = level; }
   /// Set gErrorIgnoreLevel back to previous value
   ~ChangeErrorIgnore() { gErrorIgnoreLevel = fOldLevel; }
...
class AStrm {
 ...
 public:
   ...
   AStrm &operator<<(StandardEndLine) { return operator<<("\n"); }
#endif
...
class Strm : public AStrm {
 public:
   ...
   Strm(const char *what, const char *where, const char *file, int line, std::ostream &stream, bool ignore) : AStrm(&stream), fIgnore(ignore) {
      /*!
...
class Info : public AStrm {
 public:
   Info(const char *where, const char *file = "", int line = -1, bool printMidas = true) : fWhere(""), fFile(file), fLine(line), fUseMidas(printMidas) {
      fWhere += where;
...
class Error : public AStrm {
 public:
   Error(const char *where, const char *file = "", int line = -1, bool printMidas = true) : fWhere(""), fFile(file), fLine(line), fUseMidas(printMidas) {
      fWhere += where;
      fStream = new std::stringstream();
      if (!fFile.empty()) {
         if (fFile.rfind("/") < fFile.size()) {
...
class Warning : public AStrm {
 public:
   Warning(const char *where, const char *file = "", int line = -1, bool printMidas = true) : fWhere(""), fFile(file), fLine(line), fUseMidas(printMidas) {
      fWhere += where;
      fStream = new std::stringstream();
      if (!fFile.empty()) {
         if (fFile.rfind("/") < fFile.size()) {
...
class ADelayedMessagePrinter {
 protected:
   ...
   ADelayedMessagePrinter(const char *location, int32_t period = 0,
                          ...
                          const char *message = 0) : fPeriod(period), fNumErrors(0), fLocation(location), fFile(file), fLine(line) {
      if (message)
         fMessage = message;
   }
...
class DelayedMessagePrinter : public ADelayedMessagePrinter {
 public:
   ...
   DelayedMessagePrinter(const char *location, int32_t period = 0,
                         const char *file = "", int line = -1,
                         const char *message = 0) : ADelayedMessagePrinter(location, period, file, line, message) {}
...
class DelayedMessageFactory {
 public:
   ...
   ADelayedMessagePrinter *Register(U base, int code, const char *location, int32_t period,
      ...
      if (fPrinters.insert(std::make_pair(key, err)).second)
         return err;
      else
         delete err;
   ...
   void Delete(T base, int code) {
      ...
      if (i != fPrinters.end()) {
         ...
         Warning("DeleteDelayedMessagePrinter")
...
#define EXOTIC_ERR_FILE_LINE "\nFile, line: " << __FILE__ << ", " << __LINE__ << "." \
                                              << "\n"
#endif

#ifdef __MAKECINT__
#pragma link C++ function exotic::utils::AStrm::operator<< (const char *);
#pragma link C++ function exotic::utils::AStrm::operator<< (const std::string &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Char_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Short_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Int_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Long64_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const UChar_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const UShort_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const UInt_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const ULong64_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Float_t &);
#pragma link C++ function exotic::utils::AStrm::operator<< (const Double_t &);

#pragma link C++ class exotic::utils::DelayedMessagePrinter < exotic::utils::Error> + ;
#pragma link C++ class exotic::utils::DelayedMessagePrinter < exotic::utils::Warning> + ;
#pragma link C++ class exotic::utils::DelayedMessagePrinter < exotic::utils::Info> + ;
#endif

#endif // #ifndef EXOTIC_ERROR_HXX
