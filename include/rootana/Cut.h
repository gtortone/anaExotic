 ...
 * \endcode
 */
#ifndef ROOTANA_CUT_H
#define ROOTANA_CUT_H
#include <functional>
#include <algorithm>
#include <vector>
#include <TCutG.h>
...
/*!
 * Checks if a numerical valie is "valid" (!= exotic::NO_DATA)
 * 
...
public:
	...
	bool operator() () const
		{ return exotic::utils::is_valid(fV1); }
};
	...
	fXpar(xpar), fYpar(ypar), fXpoints(0), fYpoints(0)
{
	 ...
	 * <a href="http://trshare.triumf.ca/~gchristian/exotic/analyzer/html/_cut_8hxx.html">
	...
	if (fXpoints.size() && fYpoints.size()) {
		if ( (*(fXpoints.end()-1) != *(fXpoints.begin())) ||
			...
			exotic::utils::Warning("Condition2D") << "Non-closed polygon specification, points:";
		...
		exotic::utils::Warning("Condition2D") << "Empty polygon specification";
	}
}
...
/// Checks if a parameter is valid, using exotic::utils::is_valid()
template <class T1>
